...
Indented continuations are absorbed too. A wrapped continuation aligned under the marker text arrives indented (the terminal gutter plus the list's hanging indent commonly total 4+ spaces, which on its own reads as an indented code block). Within a list or blockquote seeded join, such a line is dedented and joined rather than treated as code. It still breaks if it is structural after the dedent: a nested bullet or numbered item, a fenced code opener, a table row, a heading, a log line, and so on. A standalone indented code block (one not following a marker) is unaffected, because its own first line is the indented-code lineA preserve.
